...
BEGIN
  RAISE NOTICE 'Starting process_buy_transaction with transaction_id: %', p_transaction_id;
  ...
  RAISE NOTICE 'Transaction found: user_id=%, asset_id=%, amount=%, type=%', 
    v_transaction.user_id, v_transaction.asset_id, v_transaction.amount, v_transaction.type;
  ...
  CASE v_payment_method
    WHEN 'usd_balance' THEN
      -- First, log the balance query we're about to make
      RAISE NOTICE 'Checking USD balance for user_id=% and asset_id=%',
        v_transaction.user_id, v_usd_asset_id;

      -- Get USD balance with detailed logging
      SELECT balance INTO v_user_balance
      FROM user_balances
      WHERE user_id = v_transaction.user_id
      AND asset_id = v_usd_asset_id;

      RAISE NOTICE 'Balance query result: %', v_user_balance;

      IF v_user_balance IS NULL THEN
        RAISE NOTICE 'No balance record found for user';
        -- Create initial balance record with 0
      ...
      RAISE NOTICE 'Deducting payment: current_balance=%, amount_to_deduct=%, new_balance=%',
        v_user_balance, v_total_to_pay, (v_user_balance - v_total_to_pay);
