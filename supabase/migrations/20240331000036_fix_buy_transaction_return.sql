...
-- Recreate with proper error handling and return values
...
BEGIN
  ...
  CASE v_payment_method
    WHEN 'usd_balance' THEN
      ...
      IF v_user_balance < v_total_to_pay THEN
        RETURN jsonb_build_object(
          'success', false,
          'error', format('Insufficient USD balance. Required: %s, Available: %s', v_total_to_pay, v_user_balance),
          'transaction_id', p_transaction_id
        );
      END IF;
  ...
  CASE v_transaction.asset_type
    ...
    WHEN 'debt' THEN
      IF v_is_pool_asset THEN
        ...
        IF p_pool_id IS NULL THEN
          RETURN jsonb_build_object(
            'success', false,
            'error', 'Pool ID is required for pool debt asset purchases',
            'transaction_id', p_transaction_id
          );
        END IF;
    ...
    ELSE
      RETURN jsonb_build_object(
        'success', false,
        'error', format('Unsupported asset type: %s', v_transaction.asset_type),
        'transaction_id', p_transaction_id
      );
  END CASE;
