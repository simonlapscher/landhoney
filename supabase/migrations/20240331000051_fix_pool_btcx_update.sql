...
-- Recreate with pool BTCX balance update
...
BEGIN
    ...
    -- For pool assets, check pool balance and handle BTCX update
    IF p_pool_id IS NOT NULL THEN
        -- Get pool's main asset ID (BTCX)
        ...
        -- Calculate how much BTCX to add to pool
        -- Total USD value / BTCX price = BTCX amount
        v_btcx_amount := v_total_to_pay / p_pool_main_asset_price;

        RAISE LOG 'Pool transaction details: total_to_pay=%, btcx_price=%, btcx_amount=%',
            v_total_to_pay, p_pool_main_asset_price, v_btcx_amount;
        ...
        VALUES (p_pool_id, v_pool_main_asset_id, v_btcx_amount)
        ON CONFLICT (pool_id, asset_id) 
        DO UPDATE SET 
            balance = pool_assets.balance + v_btcx_amount,
            updated_at = NOW();
