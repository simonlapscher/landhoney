...
-- Enhanced function with proper handling for all transaction types
...
BEGIN
  ...
  v_is_pool_transaction := v_pool_id IS NOT NULL;
  v_is_commodity := (v_transaction.metadata->>'is_commodity')::BOOLEAN;

  -- Verify pool main asset price is provided when needed
  ...
  BEGIN
    ...
    -- If this is a pool asset transaction
    ...
    -- Return success response with appropriate data based on transaction type
    RETURN CASE 
